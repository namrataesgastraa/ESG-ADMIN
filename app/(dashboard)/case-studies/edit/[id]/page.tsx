...
import { fetchCaseStudyByIdThunk, updateCaseStudyThunk } from '@/lib/store/slices/caseStudySlice';
import { fetchCategoriesThunk } from '@/lib/store/slices/categorySlice';
import { BrandLine } from '@/components/BrandLine';
import { Upload, ImagePlus, FileText, ChevronLeft, X } from 'lucide-react';
...
export default function EditCaseStudy() {
  ...
  useEffect(() => {
    ...
    if (id) {
      dispatch(fetchCaseStudyByIdThunk(Number(id)))
        .unwrap()
        .then(data => {
          setFormData({
            title: data.title,
          ...
          if (data.pdf_file) {
            setExistingPdfName(data.pdf_file.split('/').pop());
          }
        })
        .catch(() => toast.error('Failed to load case study data'));
    }
  }, [id, dispatch]);
  ...
  const validate = () => {
    ...
    if (!existingPdfName && !pdfFile) newErrors.pdf = 'PDF file is required';
  ...
  const handleUpdate = async () => {
    ...
    try {
      await dispatch(updateCaseStudyThunk({ id: Number(id), formData: data })).unwrap();
      toast.success('Case study updated!', { id: loadingToast });
  ...
  return (
    <div className="max-w-5xl space-y-10 pb-20">
      ...
      <div className="flex items-center gap-4">
        <BrandLine />
        <div>
          <h1 className="text-3xl font-bold text-astraa-dark tracking-tight">Edit Case Study</h1>
            ...
            >
              {pdfFile ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-gray-500 px-4 line-clamp-1">{existingPdfName}</span>
